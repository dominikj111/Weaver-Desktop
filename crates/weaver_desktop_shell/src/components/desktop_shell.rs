...
impl WidgetContent for IconGridWidget {
    fn ui(&mut self, ui: &mut egui::Ui) {
        ...
        let actual_cols = ((available.x / cell_size) as usize).max(1).min(self.columns);
                ...
                let response = ui.allocate_ui(Vec2::splat(cell_size), |ui| {
                    ui.vertical_centered(|ui| {
                        ...
                        if let Some(ref texture) = self.textures[i] {
                            ...
                            ui.painter().rect_filled(icon_rect, 8.0, Color32::from_gray(60));
                            ui.painter().text(
                                icon_rect.center(),
                                Align2::CENTER_CENTER,
                                "📁",
...
impl WidgetContent for DesktopImageWidget {
    fn ui(&mut self, ui: &mut egui::Ui) {
        ...
        ui.painter().rect_filled(rect, self.border_radius, Color32::from_gray(40));
        ...
        if let Some(ref title) = self.title {
            ...
            ui.painter().rect_filled(
                title_rect,
                egui::CornerRadius { nw: 0, ne: 0, sw: bottom_radius, se: bottom_radius },
...
impl DesktopShell {
        ...
        Widget::column("desktop")
            ...
            .child(
                Widget::row("bottom-bar")
                    ...
                    .child(Widget::leaf("status", StatusText::new("Ready")))
                    .child(Widget::leaf("version", VersionLabel::new())),
            )
    }
    ...
    pub fn ui(&mut self, ctx: &Context, _view: impl FnOnce(&mut egui::Ui)) {
        ...
        let menu_clicked = ctx.memory_mut(|mem| {
            mem.data.get_temp::<bool>(egui::Id::new("menu_clicked")).unwrap_or(false)
        });
        if menu_clicked {
            // Clear the flag
            ctx.memory_mut(|mem| {
