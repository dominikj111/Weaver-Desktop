...
//!             .child(Widget::leaf("menu", MenuButton::new()))
//!             .child(Widget::leaf("title", Label::new("Desktop")))
//!             .child(Widget::leaf("clock", Clock::new()))
//!     )
//!     .child(
...
/// Widget - the universal building block for UI layouts.
///
/// A Widget can be a container (holding children) or a leaf (rendering custom content).
...
impl Widget {
    ...
    pub fn children(mut self, widgets: impl IntoIterator<Item = Widget>) -> Self {
        if let WidgetKind::Container(ref mut children) = self.kind {
    ...
    pub fn ui_in_rect(&mut self, ui: &mut Ui, rect: Rect) {
        ...
        if self.disabled {
            ...
            painter.rect_filled(widget_rect, self.border_radius, Color32::from_black_alpha(128));
        }
    }
